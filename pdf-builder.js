// PDF BUILDER — Mihais Restaurant (A2.1 · Lecția 7)
(function () {
    ...
    function buildCast() {
        return `<div class="cast-banner">
            ...
            <div class="cast-grid">
                <div class="cast-card"><img src="images/mihai.png" alt="Mihai"><div class="name">Mihai</div><div class="detail">Bucătar · noul meniu</div></div>
                <div class="cast-card"><img src="images/andreea.png" alt="Andreea"><div class="name">Andreea 🇷🇴</div><div class="detail">Gustă și compară</div></div>
    ...
    function buildTheory() {
        ...
        return `<h1 class="chapter">📘 1. Teorie — Mihais Restaurant: Komparativ und Superlativ</h1>` + t;
    }
    ...
    // Pentru Superlativ: prompt are deja „am ____", deci scoatem „am " din correct ca să nu dublăm
    function superlativBlock(title, instruction, data) {
        let h = `<div class="ex-block"><h3>${title}</h3><div class="instruction">${instruction}</div><div class="rezolvare-banner">✓ Rezolvare</div>`;
        data.forEach((it, i) => {
            const short = it.correct.replace(/^am\s+/i, '');
            const filled = it.prompt.replace(/_{2,}/g, `<strong style="color:#047857">${short}</strong>`);
            h += `<div class="ex-item"><span class="ex-num">${i + 1}</span><div class="ex-body"><div class="ex-q">${filled}</div>${it.hint ? `<div class="ex-explanation">💡 ${it.hint}</div>` : ''}</div></div>`;
        });
        return h + `</div>`;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += fillPromptBlock('Übung 1 — Komparativ (forma cu -er)', 'Umlaut la adjective scurte (alt→älter) · teuer pierde un -e- · neregulate (gut→besser, viel→mehr, gern→lieber, hoch→höher).', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += choiceBlock('Übung 2 — wie sau als?', 'comparativ (-er) + als (= decât) · so + adjectiv + wie (= ca, egalitate).', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += superlativBlock('Übung 3 — Superlativ (am ...-sten)', 'după -t/-d/-s/-ß/-z se adaugă -e- (am ältesten) · Umlaut rămâne (am größten) · neregulate (am besten, am meisten, am liebsten).', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillPromptBlock('Übung 4 — Forma corectă în context', 'als → comparativ (-er) · am ... → superlativ (-sten) · so ... wie → pozitiv (neschimbat!).', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += translateBlock('Übung 5 — Traducere RO → DE', 'Folosește comparativul, superlativul sau so...wie după caz.', ex5Data);
        }
        return html;
    }
        ...
        let html = `<h1 class="chapter new-section">🔁 4. Verbele lecției — Präsens · Präteritum · Perfekt</h1>
            <div class="andreea-note">
                <div class="andreea-note-content">
                    ...
                    <div>Comparația trăiește în propoziții cu verbe: sein + als (Andreea ist jünger als Mihai), essen/trinken + lieber, fahren + am schnellsten (Perfekt cu SEIN), finden + am ...-sten, machen + mehr/am meisten. Verbele TARI sunt verificate pe PONS.</div>
                </div>
            </div>`;
